...
def _get_currence_resource(resource, transform=(lambda x: x)):
    #requests.get(somepageAPI)...json()
    data = {
        'items': [
            {'code': 'usd', 'amount_to_usd': 1.00},
            {'code': 'gpb', 'amount_to_usd': 1.21},
...
def calculate_in_all_currencies(usd_amount):
    print("-- {} USD in all currencies __".format(usd_amount))
    for currency in get_currencies():
        print("In {}: {}".format(currency.code, currency.in_currency(usd_amount)))
